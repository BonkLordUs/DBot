...
class Authorization:
    ...
    def signin_by_access_token(self, token: str) -> int:
        ...
        authorized = self.client._get_data("authorized", timeout=10)
        if authorized["command"] == "err":
            error_msg = authorized.get("msg", "Unknown error")
            raise Exception(f"Auth error: {error_msg}")
        if authorized["command"] == "timeout":
            raise Exception("Auth timeout - server not responding")
        self.client.uid = authorized["id"]
        self.client.logger.debug(f"{self.client.tag}: Success auth")
        data = self.client._get_data("uu", timeout=10)
        while data["k"] != "dtp":
