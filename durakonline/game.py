...
class Game:
    ...
    def create(self, bet, password: str = "", players: int = 6,
        deck: int = 24, fast: bool = True, sw: bool = True,
        nb: bool = True, ch: bool = True, dr: bool = True) -> objects.Game:
        
        # Отправляем команду создания игры
        ...
        if data["command"] == 'err':
            ...
            print(f"[ERROR] Game creation failed: {err_msg}")
            return None
        
        if data["command"] == 'alert':
            alert_text = data.get('text', {})
            if isinstance(alert_text, dict):
                alert_text = alert_text.get('ru', alert_text.get('en', 'Unknown'))
            print(f"[ALERT] {alert_text}")
            if 'бан' in str(alert_text).lower() or 'ban' in str(alert_text).lower():
                print(f"[БАН ОБНАРУЖЕН] {alert_text}")
            return None
        
        if data["command"] == 'game':
            return objects.Game(data).Game
        
        print(f"[UNKNOWN] Unexpected response: {data.get('command', 'no command')}")
        return None
